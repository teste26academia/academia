...
const el = {
  ...
  financePreview: document.getElementById("financePreview"),
  financePreviewBase: document.getElementById("financePreviewBase"),
  financePreviewDiscount: document.getElementById("financePreviewDiscount"),
  financePreviewFinal: document.getElementById("financePreviewFinal"),
  financePreviewMode: document.getElementById("financePreviewMode"),
...
function getFinancePreviewState() {
  const plan = financePlanByKey(el.financePlano?.value);
  const baseInput = parseMoneyInput(el.financeValor?.value);
  const discountPct = Math.max(0, Math.min(100, Number(el.financeDescontoPct?.value || 0) || 0));
  const isTotalPass = plan?.key === "totalpass";
  const mode = isTotalPass ? "TotalPass" : (plan ? (plan.category === "avulsa" ? "Aula avulsa" : "Mensalidade") : "Mensalidade");
  const valorBase = isTotalPass ? 0 : (plan ? (baseInput > 0 ? baseInput : plan.base) : baseInput);
  const desconto = isTotalPass ? 0 : roundMoney(valorBase * (discountPct / 100));
  const valorFinal = roundMoney(Math.max(valorBase - desconto, 0));
  return { plan, mode, valorBase, descontoPct: isTotalPass ? 0 : discountPct, desconto, valorFinal, isTotalPass };
}

function renderFinancePreview() {
  if (!el.financePreviewBase) return;
  const preview = getFinancePreviewState();
  setText(el.financePreviewBase, money(preview.valorBase));
  setText(el.financePreviewDiscount, money(preview.desconto));
  setText(el.financePreviewFinal, money(preview.valorFinal));
  setText(el.financePreviewMode, preview.mode);
  if (el.financeStatus) {
    el.financeStatus.value = preview.isTotalPass ? "pago" : (el.financeStatus.value || "pendente");
  }
  if (el.financeValor && preview.isTotalPass) {
    el.financeValor.value = formatMoneyInputValue(0);
  }
  if (el.financeDescontoPct && preview.isTotalPass) {
    el.financeDescontoPct.value = "0";
  }
}
...
async function saveFinance() {
  ...
  if (!alunoId || (!plan && !preview.valorBase && preview.valorBase !== 0)) {
    return alert("Selecione o aluno e informe o valor.");
  }
  ...
  await addDoc(collection(db, "mensalidades"), {
    ...
    tipoCobranca,
    grupoFamilia,
    descontoPct: preview.descontoPct,
    valorBase: preview.valorBase,
    valorDesconto: preview.desconto,
    valorFinal: preview.valorFinal,
    valor: preview.valorFinal,
    vencimento,
    status: preview.isTotalPass ? "pago" : el.financeStatus.value,
  ...
  if (el.financeStatus) el.financeStatus.value = preview.isTotalPass ? "pago" : "pendente";
...
async function loginEmail() {
  const email = String(document.getElementById("emailLogin")?.value || "").trim();
  const password = String(document.getElementById("passLogin")?.value || "");
...
async function createAccount() {
  const email = String(document.getElementById("emailCreate")?.value || "").trim();
  const password = String(document.getElementById("passCreate")?.value || "");
  ...
  const nomeBase = email.split("@")[0] || "Aluno";
...
async function loadUserProfile(user) {
  ...
  if (snap.exists()) {
    ...
    state.profile = {
      ...
      nome: data.nome || user.displayName || (user.email ? user.email.split("@")[0] : "Aluno"),
      ...
      ...data,
      status: normalizeStatus(data.status || "pendente"),
  ...
  const fallback = {
    uid: user.uid,
    email: user.email || "",
    nome: user.displayName || (user.email ? user.email.split("@")[0] : "Aluno"),
    telefone: "",
    faixa: "",
    foto: "",
    observacao: "",
...
async function savePublicProfile() {
  ...
  const fotoUrl = String(document.getElementById("fotoAluno")?.value || "").trim();
  const observacao = String(document.getElementById("observacaoAluno")?.value || "").trim();
  const fotoFile = document.getElementById("fotoFile")?.files?.[0] || null;
  ...
  const currentRole = String(state.profile?.role || "aluno").trim().toLowerCase();
  const currentStatus = normalizeStatus(state.profile?.status || "pendente");
  const nextProfile = {
    uid: user.uid,
    email: user.email || state.profile?.email || "",
    nome: nome || state.profile?.nome || (user.email ? user.email.split("@")[0] : "Aluno"),
    telefone,
    faixa,
    foto,
    observacao,
    presencas: Number(state.profile?.presencas || 0),
    status: currentStatus,
    role: currentRole || "aluno",
    updatedAt: serverTimestamp()
  };
  ...
  updateCard(findProfileByUid(user.uid));
  renderHomeSummary();
...
async function deleteUser(uid) {
  ...
  if (String(data.role || "").trim().toLowerCase() === "admin") {
    throw new Error("Não é permitido excluir a conta administrativa.");
  }

  const presencesSnap = await getDocs(query(collection(db, "presencas"), where("alunoId", "==", uid)));
  for (const p of presencesSnap.docs) {
    await deleteDoc(doc(db, "presencas", p.id));
  }

  const mensalidadesSnap = await getDocs(query(collection(db, "mensalidades"), where("alunoId", "==", uid)));
  for (const m of mensalidadesSnap.docs) {
    await deleteDoc(doc(db, "mensalidades", m.id));
  }
...
async function deleteAllStudents() {
  ...
  for (const u of usersSnap.docs) {
    const data = u.data() || {};
    if (String(data.role || "").trim().toLowerCase() === "admin") continue;
...
async function saveAviso() {
  const titulo = String(el.avisoTitulo?.value || "").trim();
  const mensagem = String(el.avisoMensagem?.value || "").trim();
  const prioridade = normalizeAvisoPriority(el.avisoPrioridade?.value);
  ...
  if (el.avisoPrioridade) el.avisoPrioridade.value = "normal";
}
...
function renderUsersLists() {
  const renderList = (container, items, emptyText, mode) => {
    if (!container) return;
    container.innerHTML = "";

    if (!items.length) {
      const empty = document.createElement("div");
      empty.className = "panel-soft rounded-2xl p-4 text-zinc-300";
    ...
    items.forEach((user) => {
      const row = document.createElement("div");
      row.className = "bg-zinc-800 rounded-2xl p-3 border border-zinc-700";

      const badge = statusLabel(user.status);
      const isAdmin = String(user.role || "").trim().toLowerCase() === "admin";

      row.innerHTML = `
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="font-semibold text-white truncate">${escapeHtml(user.nome || "Sem nome")}</div>
            <div class="text-zinc-400 text-xs truncate">${escapeHtml(user.email || "-")}</div>
            <div class="text-zinc-400 text-xs mt-1">Presenças: ${Number(user.presencas || 0)}</div>
          </div>
          <span class="px-2 py-1 rounded-full text-[11px] border ${badge === "Aprovado" ? "text-emerald-300 border-emerald-500/30 bg-emerald-500/10" : badge === "Inativo" ? "text-zinc-300 border-zinc-500/30 bg-zinc-500/10" : "text-yellow-300 border-yellow-500/30 bg-yellow-500/10"}">${badge}</span>
        </div>
        <div class="mt-3 flex flex-wrap gap-2">
          ${mode === "pending" ? `<button data-approve="${escapeHtml(user.uid)}" class="flex-1 min-w-[140px] bg-yellow-600 hover:bg-yellow-500 rounded-xl py-2 font-semibold text-sm">Aprovar aluno</button>` : `<button data-revoke="${escapeHtml(user.uid)}" class="flex-1 min-w-[140px] bg-yellow-700 hover:bg-yellow-600 rounded-xl py-2 font-semibold text-sm">Voltar para pendente</button>`}
          <button data-delete-user="${escapeHtml(user.uid)}" class="flex-1 min-w-[140px] bg-red-700 hover:bg-red-600 rounded-xl py-2 font-semibold text-sm ${isAdmin ? "opacity-50 pointer-events-none" : ""}">Excluir aluno</button>
        </div>
      `;
  ...
  renderList(el.pendingList, state.pending, "Nenhum aluno pendente.", "pending");
  renderList(el.approvedList, state.approved, "Nenhum aluno aprovado.", "approved");

  const approvedSelectable = state.approved.filter((u) => normalizeStatus(u.status) === "aprovado");
  const allSelectable = [...state.approved, ...state.pending];
  const fillSelect = (selectEl, items, placeholder) => {
    ...
    if (current && items.some((u) => u.uid === current)) selectEl.value = current;
  };

  fillSelect(el.presenceAluno, approvedSelectable.length ? approvedSelectable : allSelectable, "Nenhum aluno aprovado");
  fillSelect(el.financeAluno, approvedSelectable.length ? approvedSelectable : allSelectable, "Nenhum aluno aprovado");
...
function renderAvisos() {
  ...
  const latest = avisos.slice(0, 5);

  if (el.homeAvisosCount) {
    el.homeAvisosCount.textContent = `${latest.length} aviso${latest.length === 1 ? "" : "s"}`;
  }

  if (el.avisosStatus) {
    el.avisosStatus.textContent = latest.length
      ? `${latest.length} aviso${latest.length === 1 ? "" : "s"} publicado${latest.length === 1 ? "" : "s"}.`
      : "Nenhum aviso cadastrado ainda.";
  }

  const renderTarget = (container, withDelete) => {
    if (!container) return;
    container.innerHTML = "";
    ...
    latest.forEach((aviso) => {
      const item = document.createElement("div");
      item.className = "panel-soft rounded-2xl p-4";
      const badgeClass = avisoBadgeClass(aviso.prioridade);
      const dateLabel = formatDateTime(aviso.createdAt);

      item.innerHTML = `
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="font-semibold text-white">${escapeHtml(aviso.titulo || "Aviso")}</div>
            <div class="text-zinc-400 text-xs mt-1">${escapeHtml(dateLabel)}</div>
          </div>
          <span class="px-2 py-1 rounded-full text-[11px] border ${badgeClass}">${escapeHtml(normalizeAvisoPriority(aviso.prioridade))}</span>
        </div>
  ...
  renderTarget(el.homeAvisosList, false);
  renderTarget(el.avisosList, true);
...
function bindButtons() {
  ...
  el.financeValor?.addEventListener("input", renderFinancePreview);
  el.financeDescontoPct?.addEventListener("input", renderFinancePreview);
  el.financeGrupoFamilia?.addEventListener("input", renderFinancePreview);
  el.financeStatus?.addEventListener("change", renderFinancePreview);
  ...
  el.pendingList?.addEventListener("click", async (event) => {
    ...
    if (approveBtn) {
      try { await approveUser(approveBtn.dataset.approve); } catch (error) { alert("Erro ao aprovar: " + (error?.message || error)); }
      return;
    }
    const deleteBtn = event.target.closest("button[data-delete-user]");
    if (deleteBtn) {
      try { await deleteUser(deleteBtn.dataset.deleteUser); } catch (error) { alert("Erro ao excluir aluno: " + (error?.message || error)); }
    }
  });
  ...
  el.approvedList?.addEventListener("click", async (event) => {
    ...
    if (revokeBtn) {
      try { await revokeUser(revokeBtn.dataset.revoke); } catch (error) { alert("Erro ao voltar para pendente: " + (error?.message || error)); }
      return;
    }
    const deleteBtn = event.target.closest("button[data-delete-user]");
    if (deleteBtn) {
  ...
  el.avisosList?.addEventListener("click", async (event) => {
