...
function renderAdminLists() {
  ...
  el.pendingList.innerHTML = pending.length ? "" : '<div class="text-zinc-400">Nenhum cadastro pendente.</div>';
  el.approvedList.innerHTML = approved.length ? "" : '<div class="text-zinc-400">Nenhum aluno aprovado.</div>';
  ...
  pending.forEach((p) => {
    ...
    card.innerHTML = `
      ...
      <div class="text-zinc-300 text-sm">Status: ${escapeHtml(p.status || "pendente")}</div>
...
async function approveUser(uid) {
  ...
  await updateDoc(ref, { status: "aprovado", role: "aluno" });
}

async function revokeUser(uid) {
  const ref = doc(db, "users", uid);
  await updateDoc(ref, { status: "pendente", role: "aluno" });
}
...
function attachAuthUI() {
  document.getElementById("sendCodeBtn").addEventListener("click", async () => {
    try {
      ...
      console.error(error);
      alert("Erro ao enviar código: " + (error?.message || error));
    }
  });
  ...
  document.getElementById("verifyCodeBtn").addEventListener("click", async () => {
    try {
      ...
      console.error(error);
      alert("Erro ao confirmar código: " + (error?.message || error));
    }
  });
...
async function refreshCollections() {
  try {
    ...
    state.alunos = all.filter(u => u.status === "aprovado");
...
onAuthStateChanged(auth, async (user) => {
  ...
  document.getElementById("authStatus").textContent = user ? `Logado: ${user.phoneNumber || user.uid}` : "Desconectado";
  if (user) {
