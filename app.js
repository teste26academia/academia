...
function updateStats() {
  ...
  const pago = state.mensalidades.reduce((sum, f) => {
    const valor = Number(String(f.valor || "0").replace(/[^\d,.-]/g, "").replace(",", "."));
    return sum + ((String(f.status || "pendente").trim().toLowerCase() === "pago" && !Number.isNaN(valor)) ? valor : 0);
  }, 0);
...
function renderFinance() {
  ...
  state.mensalidades.forEach((f) => {
    ...
    totalPrevisto += valor;
    if (String(f.status || "pendente").trim().toLowerCase() === "pago") totalPago += valor;
  ...
  const abertoTexto = state.mensalidades.length ? money(totalPrevisto - totalPago) : "Sem lançamentos";
...
function renderAvisos() {
  const sorted = [...state.avisos].sort((a, b) => toMillisSafe(b.createdAt) - toMillisSafe(a.createdAt));
  const recent = sorted.slice(0, 3);

  if (el.homeAvisosCount) {
    el.homeAvisosCount.textContent = `${sorted.length} aviso${sorted.length === 1 ? "" : "s"}`;
  }

  if (el.homeAvisosList) {
    el.homeAvisosList.innerHTML = "";
    if (!recent.length) {
      el.homeAvisosList.innerHTML = '<div class="panel-soft rounded-2xl p-4 text-zinc-400 text-sm">Nenhum aviso publicado ainda.</div>';
    } else {
      recent.forEach((aviso) => {
        ...
        item.innerHTML = `
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              ...
              <div class="text-zinc-300 text-sm mt-2 whitespace-pre-line">${escapeHtml(aviso.mensagem || "")}</div>
            </div>
            <span class="px-3 py-1 rounded-full text-[11px] border whitespace-nowrap ${avisoBadgeClass(aviso.prioridade)}">${escapeHtml(normalizeAvisoPriority(aviso.prioridade))}</span>
          </div>
          <div class="mt-2 text-zinc-500 text-xs">${escapeHtml(formatDateTime(aviso.createdAt))}</div>
        `;
        el.homeAvisosList.appendChild(item);
      });
    }
  }
  ...
  if (el.avisosList) {
    ...
    if (!sorted.length) {
      el.avisosList.innerHTML = '<div class="text-zinc-500 text-sm">Ainda não há avisos publicados.</div>';
    } else {
      sorted.forEach((aviso) => {
        const item = document.createElement("div");
        item.className = "bg-zinc-800/80 border border-zinc-700 rounded-2xl p-3 shadow-sm";
        item.innerHTML = `
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <div class="font-semibold text-white">${escapeHtml(aviso.titulo || "Aviso")}</div>
              <div class="text-zinc-300 text-sm mt-2 whitespace-pre-line">${escapeHtml(aviso.mensagem || "")}</div>
              <div class="text-zinc-500 text-xs mt-2">${escapeHtml(formatDateTime(aviso.createdAt))}</div>
            </div>
            <span class="px-3 py-1 rounded-full text-[11px] border whitespace-nowrap ${avisoBadgeClass(aviso.prioridade)}">${escapeHtml(normalizeAvisoPriority(aviso.prioridade))}</span>
          </div>
          <div class="mt-3 flex justify-end">
            <button data-delete-aviso="${escapeHtml(aviso.id || "")}" class="bg-red-700 hover:bg-red-600 px-3 py-2 rounded-xl font-semibold text-sm">Excluir</button>
          </div>
        `;
        item.querySelector('button[data-delete-aviso]')?.addEventListener("click", async () => {
          await deleteAviso(aviso.id);
        });
        el.avisosList.appendChild(item);
      });
    }
  }

  if (el.avisosStatus) {
    el.avisosStatus.textContent = sorted.length
      ? `Há ${sorted.length} aviso${sorted.length === 1 ? "" : "s"} publicado${sorted.length === 1 ? "" : "s"}.`
...
async function saveAviso() {
  ...
  const prioridade = String(el.avisoPrioridade?.value || "normal").trim().toLowerCase();
  ...
  await addDoc(collection(db, "avisos"), {
    ...
    authorEmail: state.currentUser?.email || null
  });
  ...
  if (el.avisosStatus) el.avisosStatus.textContent = "Aviso publicado com sucesso.";
}
