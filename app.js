...
async function changePassword() {
  ...
  if (!user?.email) {
    alert("Faça login novamente para trocar a senha.");
    return;
  }
  ...
  try {
    ...
    if (code === "auth/requires-recent-login") {
      alert("Por segurança, faça login novamente e tente outra vez.");
      return;
    }
    alert("Erro ao trocar senha: " + (error?.message || error));
  }
}
...
function bindButtons() {
  ...
  document.getElementById("backFromCreate")?.addEventListener("click", () => { clearEditMode(); show("home"); });
  ...
  document.getElementById("editProfileBtn")?.addEventListener("click", () => { clearEditMode(); fillProfileForm(state.profile || {}); show("create"); });
  ...
  document.getElementById("btnChangePassword")?.addEventListener("click", async () => { try { await changePassword(); } catch (error) { alert("Erro ao trocar senha: " + (error?.message || error)); } });
