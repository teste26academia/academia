...
function isApproved(profile) {
  return String(profile?.status || "").toLowerCase() === "aprovado";
}
...
function renderProfilePreview(profile) {
  ...
  el.profilePreview.innerHTML = `
    <div class="space-y-2">
      ...
      <div><b>Telefone:</b> ${escapeHtml(profile.telefone || "-")}</div>\n      <div><b>Nascimento:</b> ${escapeHtml(profile.nascimento || "-")}</div>\n      <div><b>CPF/RG:</b> ${escapeHtml(profile.cpf || "-")}</div>\n      <div><b>Endereço:</b> ${escapeHtml(profile.endereco || "-")}</div>
...
function renderStudentArea(profile) {
  if (!profile) {
    ...
    if (el.approvalBanner) {
      el.approvalBanner.className = "bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-4 text-yellow-300 text-sm";
      el.approvalBanner.textContent = "Aguardando aprovação do admin.";
    }
    return;
  }
  ...
  el.studentStatus.textContent = approved ? "aprovado" : (profile.status || "pendente");
  el.studentFaixa.textContent = profile.faixa || "-";
  el.studentPresencas.textContent = String(profile.presencas || 0);

  if (el.approvalBanner) {
    if (approved) {
      el.approvalBanner.className = "bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-4 text-emerald-300 text-sm";
      el.approvalBanner.textContent = "Aprovado! Carteirinha e presença liberadas.";
    } else {
      el.approvalBanner.className = "bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-4 text-yellow-300 text-sm";
      el.approvalBanner.textContent = "Aguardando aprovação do admin.";
    }
...
function renderUsersLists() {
  ...
  state.pending.forEach((u) => {
    ...
    card.innerHTML = `
      ...
      <div class="text-zinc-300 text-sm">Telefone: ${escapeHtml(u.telefone || "-")}</div>\n      <div class="text-zinc-300 text-sm">Nascimento: ${escapeHtml(u.nascimento || "-")}</div>\n      <div class="text-zinc-300 text-sm">CPF/RG: ${escapeHtml(u.cpf || "-")}</div>\n      <div class="text-zinc-300 text-sm">Endereço: ${escapeHtml(u.endereco || "-")}</div>
      <div class="text-zinc-300 text-sm">Faixa: ${escapeHtml(u.faixa || "-")}</div>
      <div class="mt-3 grid grid-cols-2 gap-2">\n        <button data-approve="${u.uid}" class="bg-emerald-700 hover:bg-emerald-600 px-4 py-3 rounded-xl font-semibold">Aprovar</button>\n        <button data-delete="${u.uid}" class="border border-red-400 bg-red-700 hover:bg-red-600 px-4 py-3 rounded-xl font-semibold">Excluir aluno</button>\n      </div>
    `;
  ...
  state.approved.forEach((u) => {
    ...
    card.innerHTML = `
      ...
      <div class="text-emerald-300 text-xs mb-2">Aprovado - carteirinha e presença liberadas</div>
      <div class="text-zinc-300 text-sm">E-mail: ${escapeHtml(u.email || "-")}</div>
      <div class="text-zinc-300 text-sm">Telefone: ${escapeHtml(u.telefone || "-")}</div>\n      <div class="text-zinc-300 text-sm">Nascimento: ${escapeHtml(u.nascimento || "-")}</div>\n      <div class="text-zinc-300 text-sm">CPF/RG: ${escapeHtml(u.cpf || "-")}</div>\n      <div class="text-zinc-300 text-sm">Endereço: ${escapeHtml(u.endereco || "-")}</div>
      <div class="text-zinc-300 text-sm">Faixa: ${escapeHtml(u.faixa || "-")}</div>
      <div class="text-zinc-300 text-sm">Presenças: ${Number(u.presencas || 0)}</div>
      <div class="mt-3 grid grid-cols-2 gap-2">\n        <button data-revoke="${u.uid}" class="bg-zinc-700 hover:bg-zinc-600 px-4 py-3 rounded-xl font-semibold">Pendente</button>\n        <button data-delete="${u.uid}" class="border border-red-400 bg-red-700 hover:bg-red-600 px-4 py-3 rounded-xl font-semibold">Excluir aluno</button>\n      </div>
    `;
...
function renderPresences() {
  ...
  document.querySelectorAll("[data-delete]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const ok = confirm("Excluir aluno aluno? O cadastro será removido.");
      if (!ok) return;

      try {
        await deleteDoc(doc(db, "users", btn.dataset.delete));

        state.pending = state.pending.filter(u => u.uid !== btn.dataset.delete);
        state.approved = state.approved.filter(u => u.uid !== btn.dataset.delete);
