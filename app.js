...
function show(section) {
  ...
  if (section !== "admin") {
    ...
    el.qrStatus.textContent = "Toque no quadro do leitor QR para iniciar a câmera.";
  }
...
function renderUsersLists() {
  if (!canAccessAdmin()) {
    if (el.presenceAluno) {
      ...
      if (state.profile) {
        const opt = document.createElement("option");
        opt.value = state.profile.uid;
        opt.textContent = `${state.profile.nome || "Aluno"} (${statusLabel(state.profile.status)})`;
        el.presenceAluno.appendChild(opt);
        el.presenceAluno.value = state.profile.uid;
      }
    }
    if (el.financeAluno) {
      ...
      if (state.profile) {
        ...
        opt.value = state.profile.uid;
        opt.textContent = `${state.profile.nome || "Aluno"} (${statusLabel(state.profile.status)})`;
        el.financeAluno.appendChild(opt);
        el.financeAluno.value = state.profile.uid;
      }
    }
    return;
  }
...
function bindButtons() {
  ...
  el.qrReader?.addEventListener("click", async () => {
    if (state.currentSection !== "admin") return;
    if (state.qrStarted) {
      stopQrScanner();
      if (el.qrStatus) el.qrStatus.textContent = "Leitor QR pausado. Toque novamente para iniciar.";
      return;
    }
    try {
      startQrScanner();
  ...
  if (el.qrReader) el.qrReader.style.cursor = "pointer";
...
function watchCollections() {
  if (typeof usersUnsub === "function") { try { usersUnsub(); } catch (_) {} }
  if (typeof presencesUnsub === "function") { try { presencesUnsub(); } catch (_) {} }
  if (typeof mensalidadesUnsub === "function") { try { mensalidadesUnsub(); } catch (_) {} }
  if (typeof selfUserUnsub === "function") { try { selfUserUnsub(); } catch (_) {} }

  usersUnsub = null;
  presencesUnsub = null;
  mensalidadesUnsub = null;
  selfUserUnsub = null;
  ...
  if (isAdminUser) {
    usersUnsub = onSnapshot(collection(db, "users"), (snapshot) => {
      ...
      if (state.currentUser) {
        const mine = all.find(u => u.uid === state.currentUser.uid) || all.find(u => u.id === state.currentUser.uid);
    ...
    presencesUnsub = onSnapshot(query(collection(db, "presencas"), orderBy("createdAt", "desc")), (snapshot) => {
      state.presences = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      renderPresences();
      updateStats();
    }, () => {});

    mensalidadesUnsub = onSnapshot(query(collection(db, "mensalidades"), orderBy("createdAt", "desc")), (snapshot) => {
      state.mensalidades = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      renderFinance();
      updateStats();
    }, () => {});
  } else {
    selfUserUnsub = onSnapshot(doc(db, "users", userId), (snap) => {
      if (snap.exists()) {
        const data = snap.data() || {};
        state.profile = {
          id: snap.id,
          uid: userId,
          email: state.currentUser?.email || data.email || "",
          nome: data.nome || state.currentUser?.displayName || (state.currentUser?.email ? state.currentUser.email.split("@")[0] : "Aluno"),
      ...
      state.approved = state.profile && normalizeStatus(state.profile.status) === "aprovado" ? [state.profile] : [];
      state.pending = state.profile && normalizeStatus(state.profile.status) !== "aprovado" ? [state.profile] : [];
      renderProfilePreview(state.profile);
      renderStudentArea(state.profile);
      updateCard(findProfileByUid(state.cardUid || state.currentUser.uid));
      renderHomeSummary();
    ...
    presencesUnsub = onSnapshot(query(collection(db, "presencas"), where("alunoId", "==", userId), orderBy("createdAt", "desc")), (snapshot) => {
      state.presences = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      renderPresences();
      updateStats();
    }, () => {});

    mensalidadesUnsub = onSnapshot(query(collection(db, "mensalidades"), where("alunoId", "==", userId), orderBy("createdAt", "desc")), (snapshot) => {
      state.mensalidades = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      renderFinance();
      updateStats();
    }, () => {});
