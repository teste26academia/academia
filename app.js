...
function bindButtons() {
  ...
  document.getElementById("btnAdmin").addEventListener("click", () => show("login"));
  ...
  document.getElementById("quickAluno").addEventListener("click", () => {
    if (!state.currentUser) return alert("Faça login primeiro.");
    show("aluno");
  });
  document.getElementById("quickAdmin").addEventListener("click", () => show("login"));
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (user) {
    ...
    if (state.role === "admin" || localStorage.getItem("adminUnlocked") === "1") {
      state.role = "admin";
      show("admin");
    } else {
      show("aluno");
