...
function syncAdminControlsVisibility() {
  ...
  const ids = ["mobileAdminBtn", "btnAdmin", "adminDetails"];
  ids.forEach((id) => {
    const node = document.getElementById(id);
    if (!node) return;
    node.classList.toggle("hidden", !showAdmin);
  });
}
...
function getProfileCompletionPercent(profile) {
  if (!profile) return 0;
  const total = 4;
  const done = Math.max(0, total - getProfileMissingFields(profile).length);
...
function getGraduationSummary(profile) {
  if (!profile) return "-";
  const parts = [];
  if (profile.graduacao) parts.push(`Atual: ${profile.graduacao}`);
  if (profile.graduacaoData) parts.push(`Data: ${normalizeDateText(profile.graduacaoData)}`);
  if (profile.proximaGraduacao) parts.push(`Próx.: ${normalizeDateText(profile.proximaGraduacao)}`);
...
function countTodayPresences() {
  const today = getTodayKey();
  return (state.presences || []).filter((p) => String(p.data || "") === today).length;
}

function countCompleteProfiles() {
  return [...state.approved, ...state.pending].filter((p) => getProfileMissingFields(p).length === 0).length;
}

function countProfilesWithGraduation() {
  return [...state.approved, ...state.pending].filter((p) => String(p.graduacao || p.proximaGraduacao || p.graduacaoData || "").trim()).length;
}
...
function renderHomeSummary() {
  ...
  const homeCompletion = document.getElementById("homeCompletionPercent");
  const homeCompletionBar = document.getElementById("homeCompletionBar");
  if (homeCompletion) homeCompletion.textContent = state.profile ? `${getProfileCompletionPercent(state.profile)}%` : "0%";
  if (homeCompletionBar) homeCompletionBar.style.width = state.profile ? `${getProfileCompletionPercent(state.profile)}%` : "0%";
}
...
function updateStats() {
  ...
  setText(document.getElementById("dashCompletos"), String(countCompleteProfiles()));
  setText(document.getElementById("dashHoje"), String(countTodayPresences()));
  setText(document.getElementById("dashGraduacoes"), String(countProfilesWithGraduation()));
  setText(document.getElementById("dashAvisos"), String((state.avisos || []).length));
  setText(document.getElementById("backupCountStudents"), String(totalAlunos));
  setText(document.getElementById("backupCountPresences"), String(presencas));
  setText(document.getElementById("backupCountFinance"), String(state.mensalidades.length));
  setText(document.getElementById("backupCountAvisos"), String((state.avisos || []).length));

  setText(el.finPrevisto, money(arrecadacao));
...
function fillProfileForm(profile) {
  ...
  setVal("graduacaoAluno", profile.graduacao || "");
  setVal("graduacaoDataAluno", profile.graduacaoData || "");
  setVal("proximaGraduacaoAluno", profile.proximaGraduacao || "");
...
function clearEditMode() {
  ...
  const saveBtn = document.getElementById("savePublicProfileBtn");
  if (saveBtn) saveBtn.textContent = "Salvar cadastro";
}

function safeClearEditMode() {
  if (typeof clearEditMode === "function") {
    clearEditMode();
    return;
  }
  state.editingUid = null;
...
function refreshProfileEditorUi() {
  const saveBtn = document.getElementById("savePublicProfileBtn");
  if (saveBtn) {
    saveBtn.textContent = state.editingUid ? "Atualizar cadastro" : "Salvar cadastro";
  }
}
...
function renderProfilePreview(profile) {
  ...
  el.profilePreview.innerHTML = `
    <div class="space-y-2">
      ...
      <div><b>Graduação:</b> ${escapeHtml(getGraduationSummary(profile))}</div>
      <div><b>Status:</b> ${escapeHtml(profile.status || "-")}</div>
      <div><b>Observação:</b> ${escapeHtml(profile.observacao || "-")}</div>
      <div class="pt-2">
        <div class="flex items-center justify-between text-xs text-zinc-400"><span>Cadastro completo</span><span>${completion}%</span></div>
        <div class="mt-2 h-2 rounded-full bg-white/5 overflow-hidden"><div class="h-full bg-red-500" style="width:${completion}%"></div></div>
      </div>
    </div>`;
}
...
function renderStudentArea(profile) {
  ...
  if (!profile) {
    ...
    if (el.studentGraduation) el.studentGraduation.textContent = "-";
    if (el.studentNextGraduation) el.studentNextGraduation.textContent = "-";
    if (el.studentCompletionPercent) el.studentCompletionPercent.textContent = "0%";
    if (el.studentCompletionBar) el.studentCompletionBar.style.width = "0%";
  ...
  if (el.studentGraduation) el.studentGraduation.textContent = profile.graduacao || "-";
  if (el.studentNextGraduation) el.studentNextGraduation.textContent = normalizeDateText(profile.proximaGraduacao || profile.graduacaoData || "-");
  ...
  const completion = getProfileCompletionPercent(profile);
  if (el.studentCompletionPercent) el.studentCompletionPercent.textContent = `${completion}%`;
  if (el.studentCompletionBar) el.studentCompletionBar.style.width = `${completion}%`;

  if (el.studentPresenceHistory) {
    const history = state.presences.filter(p => p.alunoId === profile.uid).slice(0, 8);
...
function updateCard(profile) {
  ...
  if (el.cardGraduation) el.cardGraduation.textContent = profile.graduacao || "-";
  if (el.cardNextGraduation) el.cardNextGraduation.textContent = normalizeDateText(profile.proximaGraduacao || profile.graduacaoData || "-");
...
function renderUsersLists() {
  const all = canAccessAdmin() ? [...state.approved, ...state.pending] : (state.profile ? [state.profile] : []);
  ...
  const fillSelect = (selectEl, placeholder, onlyApproved = false) => {
    ...
    selectEl.innerHTML = "";
    const users = canAccessAdmin()
      ? (onlyApproved ? state.approved : all)
    ...
    const target = currentValue || (users[0]?.uid || "");
  ...
  const renderBucket = (container, users, emptyLabel, actionLabel, actionClass, actionHandler) => {
    ...
    container.innerHTML = "";
    if (!canAccessAdmin()) {
      container.innerHTML = '<div class="text-zinc-500 text-sm">Área disponível apenas para administrador.</div>';
      return;
    }
    if (!users.length) {
    ...
    users.slice(0, 12).forEach((u) => {
      ...
      row.innerHTML = `
        <div class="flex items-start justify-between gap-3">
          ...
          <span class="px-3 py-1 rounded-full text-[11px] border border-white/10 text-zinc-300 whitespace-nowrap">${escapeHtml(statusLabel(u.status))}</span>
        </div>
        <div class="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2">
          ...
          <button class="w-full rounded-xl px-3 py-2 text-sm font-semibold bg-zinc-700 hover:bg-zinc-600" data-action="edit" data-uid="${escapeHtml(u.uid || "")}">
            Editar cadastro
          </button>
      ...
      row.querySelector('button[data-action="edit"]')?.addEventListener("click", () => editUser(u.uid));
...
async function savePublicProfile() {
  ...
  const nome = String(document.getElementById("nomeAluno")?.value || "").trim();
  const telefone = String(document.getElementById("telefoneAluno")?.value || "").trim();
  const faixa = String(document.getElementById("faixaAluno")?.value || "").trim();
  const graduacao = String(document.getElementById("graduacaoAluno")?.value || "").trim();
  const graduacaoData = String(document.getElementById("graduacaoDataAluno")?.value || "").trim();
  const proximaGraduacao = String(document.getElementById("proximaGraduacaoAluno")?.value || "").trim();
  const observacao = String(document.getElementById("observacaoAluno")?.value || "").trim();
  if (!nome) return alert("Digite seu nome.");

  let foto = String(document.getElementById("fotoAluno")?.value || "").trim();
  try {
  ...
  const targetUid = canAccessAdmin() && state.editingUid ? state.editingUid : state.currentUser.uid;
  ...
  const profile = {
    uid: targetUid,
    email: existing.email || state.currentUser.email || "",
    nome,
    telefone,
    faixa,
    graduacao,
  ...
  if (canAccessAdmin() && state.editingUid && state.editingUid !== state.currentUser.uid) {
    state.editingUid = null;
    const saveBtn = document.getElementById("savePublicProfileBtn");
    if (saveBtn) saveBtn.textContent = "Salvar cadastro";
...
function serializeExportUsers(users) {
  return users.map((u) => ({
    uid: u.uid || "",
    nome: u.nome || "",
    email: u.email || "",
    ...
    graduacaoData: u.graduacaoData || "",
    proximaGraduacao: u.proximaGraduacao || "",
    status: u.status || "",
    presencas: Number(u.presencas || 0),
...
async function saveAviso() {
  ...
  const titulo = String(document.getElementById("avisoTitulo")?.value || "").trim();
  const mensagem = String(document.getElementById("avisoMensagem")?.value || "").trim();
  const prioridade = String(document.getElementById("avisoPrioridade")?.value || "normal").trim();
  if (!titulo || !mensagem) return alert("Digite o título e a mensagem do aviso.");
  ...
  const msgEl = document.getElementById("avisoMensagem");
  if (tituloEl) tituloEl.value = "";
  if (msgEl) msgEl.value = "";
  const prioridadeEl = document.getElementById("avisoPrioridade");
...
function exportBackupExcel() {
  if (!canAccessAdmin()) return alert("Área restrita ao administrador.");
  if (!window.XLSX) {
    alert("A biblioteca de Excel ainda não carregou.");
    return;
  }

  const wb = XLSX.utils.book_new();
  const approved = serializeExportUsers(state.approved);
  const pending = serializeExportUsers(state.pending);
  const presences = (state.presences || []).map((p) => ({
    id: p.id || "",
    alunoId: p.alunoId || "",
    aluno: (findProfileByUid(p.alunoId)?.nome) || "",
    aula: p.aula || "",
    data: p.data || "",
    hora: p.hora || "",
    origem: p.origem || ""
  }));
  const mensalidades = (state.mensalidades || []).map((f) => ({
    id: f.id || "",
    alunoId: f.alunoId || "",
    aluno: (findProfileByUid(f.alunoId)?.nome) || "",
    valor: f.valor || "",
    vencimento: f.vencimento || "",
    status: f.status || "",
    createdAt: f.createdAt ? toMillisSafe(f.createdAt) : ""
  }));
  const avisos = (state.avisos || []).map((a) => ({
    id: a.id || "",
    titulo: a.titulo || "",
    mensagem: a.mensagem || "",
    prioridade: a.prioridade || "",
    createdAt: a.createdAt ? toMillisSafe(a.createdAt) : ""
  }));

  const sheets = [
    ["Alunos aprovados", approved],
    ["Alunos pendentes", pending],
    ["Presenças", presences],
  ...
  XLSX.writeFile(wb, `kungfu_garra_aguia_backup_${today}.xlsx`);
}
...
async function editUser(uid) {
  ...
  try {
    ...
    fillProfileForm({
      ...
      graduacaoData: data.graduacaoData || "",
      proximaGraduacao: data.proximaGraduacao || "",
      foto: data.foto || "",
    ...
    renderProfilePreview({
      ...
      foto: data.foto || "",
      observacao: data.observacao || "",
      presencas: Number(data.presencas || 0),
      status: data.status || "pendente"
    });
  } catch (error) {
    console.error(error);
    alert("Erro ao abrir cadastro: " + (error?.message || error));
  }
}
...
function renderAvisos() {
  const avisos = [...(state.avisos || [])].sort((a, b) => toMillisSafe(b.createdAt) - toMillisSafe(a.createdAt));

  const renderTarget = (container, withDelete = false) => {
    if (!container) return;
    container.innerHTML = "";

    if (!avisos.length) {
      container.innerHTML = '<div class="text-zinc-500 text-sm">Nenhum aviso publicado ainda.</div>';
      return;
    }

    avisos.slice(0, withDelete ? 12 : 5).forEach((aviso) => {
      const item = document.createElement("div");
      item.className = "panel-soft rounded-2xl p-4";

      const priority = String(aviso.prioridade || "normal").trim().toLowerCase();
      const badgeClass = priority === "urgente"
      ...
      item.innerHTML = `
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="font-semibold text-white">${escapeHtml(aviso.titulo || "Aviso")}</div>
            <div class="text-zinc-400 text-xs mt-1">${escapeHtml(formatDateTime(aviso.createdAt))}</div>
          </div>
          <span class="px-2 py-1 rounded-full text-[11px] border ${badgeClass}">${escapeHtml(priority === "urgente" ? "Urgente" : priority === "importante" ? "Importante" : "Normal")}</span>
        </div>
        <div class="mt-3 text-zinc-300 text-sm leading-relaxed whitespace-pre-wrap">${escapeHtml(aviso.mensagem || "")}</div>
      `;

      if (withDelete) {
        const actions = document.createElement("div");
        actions.className = "mt-3 flex justify-end";
        actions.innerHTML = `<button data-delete-aviso="${escapeHtml(aviso.id)}" class="bg-red-700 hover:bg-red-600 rounded-xl px-3 py-2 font-semibold text-xs">Excluir</button>`;
  ...
  renderTarget(document.getElementById("homeAvisosList"), false);
  renderTarget(document.getElementById("avisosList"), true);

  const count = avisos.length;
  const countText = count ? `${count} aviso${count === 1 ? "" : "s"}` : "Nenhum aviso publicado ainda.";
  setText(document.getElementById("homeAvisosCount"), count ? String(count) : "0");
  setText(document.getElementById("avisosStatus"), countText);
}
...
function bindButtons() {
  ...
  document.getElementById("backFromCreate")?.addEventListener("click", () => { safeClearEditMode(); show("home"); });
  ...
  document.getElementById("savePublicProfileBtn")?.addEventListener("click", async () => { try { await savePublicProfile(); refreshProfileEditorUi(); } catch (error) { alert("Erro ao salvar cadastro: " + (error?.message || error)); } });
  ...
  document.getElementById("exportExcelBtn")?.addEventListener("click", () => { try { exportBackupExcel(); } catch (error) { alert("Erro ao exportar backup: " + (error?.message || error)); } });
  document.getElementById("cardSelect")?.addEventListener("change", updateCardFromSelect);
  document.getElementById("avisosList")?.addEventListener("click", async (event) => {
    const btn = event.target.closest("button[data-delete-aviso]");
    if (!btn) return;
    try { await deleteAviso(btn.dataset.deleteAviso); } catch (error) { alert("Erro ao excluir aviso: " + (error?.message || error)); }
  });
...
function resetCollectionWatchers() {
  collectionUnsubs.forEach((unsub) => {
...
function watchCollections() {
  resetCollectionWatchers();
  if (!state.currentUser) return;

  if (canAccessAdmin()) {
    const unsubUsers = onSnapshot(collection(db, "users"), (snapshot) => {
      const all = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      state.approved = all.filter(u => normalizeStatus(u.status) === "aprovado");
      state.pending = all.filter(u => normalizeStatus(u.status) !== "aprovado");
      updateStats();
      renderUsersLists();
      renderCardSelect();
    ...
    const unsubPres = onSnapshot(query(collection(db, "presencas"), orderBy("createdAt", "desc")), (snapshot) => {
      state.presences = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      renderPresences();
      updateStats();
    }, () => {});

    const unsubFin = onSnapshot(query(collection(db, "mensalidades"), orderBy("createdAt", "desc")), (snapshot) => {
    ...
    collectionUnsubs.push(unsubUsers, unsubPres, unsubFin, unsubAvisos);
    return;
  }
  ...
  const unsubUser = onSnapshot(doc(db, "users", uid), (snap) => {
    if (snap.exists()) {
      const data = snap.data() || {};
      state.profile = { uid, ...data, status: normalizeStatus(data.status) };
    }
    const mine = state.profile ? [state.profile] : [];
    state.approved = normalizeStatus(state.profile?.status) === "aprovado" ? mine : [];
    state.pending = normalizeStatus(state.profile?.status) !== "aprovado" ? mine : [];
    renderUsersLists();
    renderCardSelect();
    renderProfilePreview(state.profile);
    renderStudentArea(state.profile);
    updateCard(findProfileByUid(uid));
    renderHomeSummary();
    setBankStatus("Conectado");
  }, () => setBankStatus("Offline"));

  const unsubPres = onSnapshot(query(collection(db, "presencas"), where("alunoId", "==", uid)), (snapshot) => {
    state.presences = snapshot.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => toMillisSafe(b.createdAt) - toMillisSafe(a.createdAt));
    renderPresences();
    updateStats();
  }, () => {});

  const unsubFin = onSnapshot(query(collection(db, "mensalidades"), where("alunoId", "==", uid)), (snapshot) => {
    state.mensalidades = snapshot.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => toMillisSafe(b.createdAt) - toMillisSafe(a.createdAt));
    renderFinance();
    updateStats();
  }, () => {});
  ...
  collectionUnsubs.push(unsubUser, unsubPres, unsubFin, unsubAvisos);
}
