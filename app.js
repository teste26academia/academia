...
function renderUsersLists() {
  ...
  fillSelect(el.presenceAluno, "Nenhum aluno cadastrado", false);
  fillSelect(el.financeAluno, "Nenhum aluno cadastrado", false);
  ...
  const renderBucket = (container, users, emptyLabel, actionLabel, actionClass, actionHandler) => {
    ...
    users.slice(0, 12).forEach((u) => {
      const row = document.createElement("div");
      row.className = "bg-zinc-800/80 border border-zinc-700 rounded-2xl p-3";
      row.innerHTML = `
        <div class="flex items-start justify-between gap-3">
          <div>
            <div class="font-semibold text-white">${escapeHtml(u.nome || "Sem nome")}</div>
            <div class="text-zinc-400 text-xs mt-1">${escapeHtml(u.email || "-")}</div>
            <div class="text-zinc-400 text-xs mt-1">Presenças: ${Number(u.presencas || 0)}</div>
          </div>
          <span class="px-3 py-1 rounded-full text-[11px] border border-white/10 text-zinc-300">${escapeHtml(u.status || "pendente")}</span>
        </div>
        <div class="mt-3">
          <button class="${actionClass} w-full rounded-xl px-3 py-2 text-sm font-semibold" data-uid="${escapeHtml(u.uid || "")}">
            ${actionLabel}
          </button>
        </div>
      `;
      const btn = row.querySelector("button[data-uid]");
      btn?.addEventListener("click", () => actionHandler(u.uid));
      container.appendChild(row);
    });
  };

  renderBucket(el.pendingList, state.pending, "Nenhum cadastro pendente.", "Aprovar aluno", "bg-emerald-700 hover:bg-emerald-600", approveUser);
