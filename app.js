...
async function sendResetPassword() {
  ...
  const email = emailInput?.value?.trim() || prompt("Digite seu e-mail para receber o link de redefinição:");
  if (!email) return;
  try {
    await sendPasswordResetEmail(auth, email);
    alert("Enviamos um e-mail para redefinir sua senha.");
  } catch (error) {
    alert("Erro ao enviar recuperação: " + (error?.message || error));
  }
}
...
async function changePassword() {
  ...
  if (!currentPassword || !newPassword || !confirmPassword) {
    return alert("Preencha a senha atual, a nova senha e a confirmação.");
  }
  ...
  if (!user?.email) return alert("Faça login novamente e tente outra vez.");
  ...
  try {
    ...
    alert("Erro ao alterar senha: " + (error?.message || error));
  }
}
...
function bindButtons() {
  ...
  document.getElementById("btnResetPassword")?.addEventListener("click", sendResetPassword);
