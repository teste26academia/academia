...
const PLANOS = [
  { key: "1x", label: "1 aula por semana", valor: 60 },
  { key: "2x", label: "2 aulas por semana", valor: 96 },
  { key: "3x", label: "3 aulas por semana", valor: 115 },
  { key: "4x", label: "4 aulas por semana", valor: 135 },
  { key: "avulsa", label: "Aula avulsa", valor: 15 }
];
...
const el = {
  ...
  planoAluno: document.getElementById("planoAluno"),
  grupoFamiliarAluno: document.getElementById("grupoFamiliarAluno"),
  financePlano: document.getElementById("financePlano"),
  financeFamilyGroup: document.getElementById("financeFamilyGroup"),
  financeFamilyDiscountPct: document.getElementById("financeFamilyDiscountPct"),
  financeCalcPreview: document.getElementById("financeCalcPreview"),
  finAtrasados: document.getElementById("finAtrasados")
};
...
function getPlanByKey(key) {
  return PLANOS.find((p) => p.key === String(key ?? "").trim()) || null;
}

function getPlanLabel(keyOrLabel) {
  const key = String(keyOrLabel ?? "").trim();
  const byKey = getPlanByKey(key);
  if (byKey) return byKey.label;
  const byLabel = PLANOS.find((p) => p.label.toLowerCase() === key.toLowerCase());
  return byLabel ? byLabel.label : key || "-";
}

function getProfilePlanKey(profile) {
  const raw = String(profile?.planoKey || profile?.plano || profile?.planoLabel || "").trim();
  const byKey = getPlanByKey(raw);
  if (byKey) return byKey.key;
  const byLabel = PLANOS.find((p) => p.label.toLowerCase() === raw.toLowerCase());
  if (byLabel) return byLabel.key;
  return raw;
}

function getProfileFamilyGroup(profile) {
  return String(profile?.familyGroup || profile?.grupoFamiliar || "").trim();
}

function getProfileFamilyDiscountPct(profile) {
  const direct = Number(profile?.familyDiscountPct || 0);
  if (Number.isFinite(direct) && direct > 0) return direct;
  const group = normalizeGroup(getProfileFamilyGroup(profile));
  if (!group) return 0;
  const pool = [...state.approved, ...state.pending];
  const match = pool.find((u) => normalizeGroup(getProfileFamilyGroup(u)) === group && Number(u.familyDiscountPct || 0) > 0);
  return match ? Number(match.familyDiscountPct || 0) : 0;
}

function calcMonthlyForProfile(profile, planKeyOverride = null, manualFinalOverride = null) {
  const planKey = planKeyOverride || getProfilePlanKey(profile) || "1x";
  const plan = getPlanByKey(planKey) || PLANOS[0];
  const base = Number(plan?.valor || 0);
  const discountPct = getProfileFamilyDiscountPct(profile);
  const discountValue = Math.max(0, (base * discountPct) / 100);
  const calcFinal = Math.max(0, base - discountValue);
  const manualFinal = parseMoneyInput(manualFinalOverride);
  const final = manualFinal > 0 ? manualFinal : calcFinal;
  return {
    planKey: plan.key,
...
function findUserByUid(uid) {
  return [...state.approved, ...state.pending, state.profile].filter(Boolean).find((u) => u.uid === uid) || null;
}

function updateFinancePreview() {
  const selectedUid = el.financeAluno?.value || "";
  const profile = findUserByUid(selectedUid);
  const planKey = el.financePlano?.value || getProfilePlanKey(profile) || "1x";
  const manualFinal = el.financeValor?.value || "";
  const calc = calcMonthlyForProfile(profile, planKey, manualFinal);

  if (el.financeValor && !manualFinal) {
    el.financeValor.value = calc.final ? calc.final.toFixed(2) : "";
  }
  if (el.financeFamilyGroup && profile) {
    el.financeFamilyGroup.value = getProfileFamilyGroup(profile) || "";
  }
  if (el.financeFamilyDiscountPct && profile) {
    const pct = getProfileFamilyDiscountPct(profile);
    el.financeFamilyDiscountPct.value = pct ? String(pct) : "";
  }
  if (el.financeCalcPreview) {
    el.financeCalcPreview.innerHTML = `
      <div class="grid grid-cols-2 gap-2 text-sm">
        <div class="panel-soft rounded-2xl p-3">
          <div class="text-zinc-500 text-[11px]">Valor base</div>
          <div class="font-black text-white mt-1">${money(calc.base)}</div>
        </div>
        <div class="panel-soft rounded-2xl p-3">
          <div class="text-zinc-500 text-[11px]">Desconto familiar</div>
          <div class="font-black text-white mt-1">${calc.discountPct ? `${calc.discountPct}%` : "-"}</div>
        </div>
        <div class="panel-soft rounded-2xl p-3">
          <div class="text-zinc-500 text-[11px]">Valor do desconto</div>
          <div class="font-black text-white mt-1">${money(calc.discountValue)}</div>
        </div>
        <div class="panel-soft rounded-2xl p-3">
          <div class="text-zinc-500 text-[11px]">Valor final</div>
          <div class="font-black text-emerald-400 mt-1">${money(calc.final)}</div>
        </div>
      </div>
    `;
  }
...
function refreshPlanSelects() {
  ["planoAluno", "financePlano"].forEach((id) => {
    ...
    PLANOS.forEach((plan) => {
      ...
      opt.value = plan.key;
      opt.textContent = `${plan.label} — ${money(plan.valor)}`;
      select.appendChild(opt);
    });
    if (current && PLANOS.some((p) => p.key === current)) select.value = current;
  });
}
...
function getProfileMissingFields(profile) {
  ...
  if (!getProfilePlanKey(profile)) missing.push("Plano");
...
function fillProfileForm(profile) {
  ...
  setVal("planoAluno", getProfilePlanKey(profile) || "");
  setVal("grupoFamiliarAluno", getProfileFamilyGroup(profile) || "");
...
function renderProfilePreview(profile) {
  ...
  const planLabel = getPlanLabel(getProfilePlanKey(profile));
  const familyGroup = getProfileFamilyGroup(profile) || "-";
  el.profilePreview.innerHTML = `
    <div class="space-y-2">
      ...
      <div><b>Plano:</b> ${escapeHtml(planLabel)}</div>
      <div><b>Grupo familiar:</b> ${escapeHtml(familyGroup)}</div>
...
function getStudentMonthlyStatus(profile) {
  ...
  if (fee) {
    const finalValue = Number(fee.valorFinal ?? fee.valor ?? 0);
    return `${fee.status === "pago" ? "Em dia" : "Pendente"} • ${money(finalValue)}`;
  }
  const calc = calcMonthlyForProfile(profile);
  return `${calc.planLabel} • ${money(calc.final)}`;
}
...
function renderStudentArea(profile) {
  ...
  el.studentProfile.innerHTML = `
    <div class="space-y-2">
      ...
      <div><b>Mensalidade:</b> ${money(calc.final)}</div>
      <div><b>Observação:</b> ${escapeHtml(profile.observacao || "-")}</div>
    </div>`;
...
function renderFinance() {
  ...
  state.mensalidades.forEach((f) => {
    ...
    const valor = Number(f.valorFinal ?? f.valor ?? 0);
    const venc = String(f.vencimento || "");
    const status = String(f.status || "pendente");
    const vencido = venc && status !== "pago" && venc < new Date().toISOString().slice(0, 10);

    totalPrevisto += Number.isNaN(valor) ? 0 : valor;
    if (status === "pago") totalPago += Number.isNaN(valor) ? 0 : valor;
    if (vencido) totalAtrasado += Number.isNaN(valor) ? 0 : valor;

    const planLabel = f.planoLabel || getPlanLabel(f.planoKey || "");
    const discountPct = Number(f.descontoPct || 0);
    const group = f.familyGroup || "-";
    const base = Number(f.valorBase ?? valor);
    const finalValue = Number.isNaN(valor) ? base : valor;
    ...
    card.innerHTML = `
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
          ...
          <div class="text-zinc-300">Grupo familiar: ${escapeHtml(group)}</div>
          <div class="text-zinc-300">Valor base: ${money(base)}</div>
          <div class="text-zinc-300">Desconto: ${discountPct ? `${discountPct}%` : "-"}</div>
          <div class="text-zinc-300">Valor final: ${money(finalValue)}</div>
          <div class="text-zinc-300">Vencimento: ${escapeHtml(f.vencimento || "-")}</div>
          <div class="text-zinc-300">Status: <span class="${status === "pago" ? "text-emerald-400" : vencido ? "text-red-400" : "text-yellow-400"}">${escapeHtml(status)}</span></div>
        </div>
  ...
  setText(el.finAberto, money(Math.max(totalPrevisto - totalPago, 0)));
  if (el.finAtrasados) setText(el.finAtrasados, money(totalAtrasado));
      ...
      row.innerHTML = `
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            ...
            <div class="text-zinc-400 text-xs mt-1">Plano: ${escapeHtml(getPlanLabel(getProfilePlanKey(u)))}</div>
            <div class="text-zinc-400 text-xs mt-1">Grupo: ${escapeHtml(getProfileFamilyGroup(u) || "-")}</div>
...
async function savePublicProfile() {
  ...
  const planoKey = document.getElementById("planoAluno").value.trim();
  const grupoFamiliar = document.getElementById("grupoFamiliarAluno").value.trim();
  const observacao = document.getElementById("observacaoAluno").value.trim();
  if (!nome) return alert("Digite seu nome.");
  if (!planoKey) return alert("Selecione um plano.");
  ...
  const profile = {
    ...
    faixa,
    planoKey,
    planoLabel: getPlanLabel(planoKey),
    familyGroup: grupoFamiliar,
    familyDiscountPct: Number(state.profile?.familyDiscountPct || 0),
    foto,
    observacao,
...
async function saveFinance() {
  ...
  const selectedProfile = findUserByUid(alunoId);
  const planoKey = el.financePlano.value || getProfilePlanKey(selectedProfile) || "1x";
  const familyGroup = (el.financeFamilyGroup.value || getProfileFamilyGroup(selectedProfile) || "").trim();
  const familyDiscountPct = Number(el.financeFamilyDiscountPct.value || selectedProfile?.familyDiscountPct || 0);
  const vencimento = el.financeVencimento.value;
  const status = el.financeStatus.value;
  if (!alunoId) return alert("Selecione o aluno.");
  const calc = calcMonthlyForProfile({ ...selectedProfile, familyGroup, familyDiscountPct, planoKey }, planoKey, el.financeValor.value.trim());
  const valorFinal = calc.final;
  const valorBase = calc.base;
  const descontoValor = Math.max(0, valorBase - valorFinal);

  if (selectedProfile) {
    await updateDoc(doc(db, "users", alunoId), {
      planoKey,
      planoLabel: calc.planLabel,
      familyGroup,
      familyDiscountPct,
      updatedAt: serverTimestamp()
    });
...
async function applyFamilyDiscountToGroup() {
  const alunoId = el.financeAluno.value;
  const selectedProfile = findUserByUid(alunoId);
  const group = (el.financeFamilyGroup.value || getProfileFamilyGroup(selectedProfile) || "").trim();
  const pct = Number(el.financeFamilyDiscountPct.value || selectedProfile?.familyDiscountPct || 0);
  if (!alunoId) return alert("Selecione um aluno.");
  if (!group) return alert("Informe o grupo familiar.");
  if (!pct || pct < 0) return alert("Informe a porcentagem de desconto.");

  const targets = [...state.approved, ...state.pending].filter((u) => normalizeGroup(getProfileFamilyGroup(u)) === normalizeGroup(group));
  if (!targets.length) return alert("Nenhum aluno encontrado nesse grupo.");

  await Promise.all(targets.map((u) => updateDoc(doc(db, "users", u.uid), {
    familyGroup: group,
    familyDiscountPct: pct,
    updatedAt: serverTimestamp()
  })));

  alert(`Desconto de ${pct}% aplicado ao grupo ${group}.`);
}
...
function bindButtons() {
  ...
  document.getElementById("applyFamilyDiscountBtn")?.addEventListener("click", async () => { try { await applyFamilyDiscountToGroup(); } catch (error) { alert("Erro ao aplicar desconto: " + (error?.message || error)); } });
  ...
  document.getElementById("financeAluno")?.addEventListener("change", updateFinancePreview);
  document.getElementById("financePlano")?.addEventListener("change", updateFinancePreview);
  document.getElementById("financeFamilyGroup")?.addEventListener("input", updateFinancePreview);
  document.getElementById("financeFamilyDiscountPct")?.addEventListener("input", updateFinancePreview);
  document.getElementById("planoAluno")?.addEventListener("change", () => {
    if (state.profile) {
      state.profile.planoKey = document.getElementById("planoAluno")?.value || state.profile.planoKey;
      state.profile.planoLabel = getPlanLabel(state.profile.planoKey);
      renderProfilePreview(state.profile);
      renderStudentArea(state.profile);
      updateCard(state.profile);
    }
