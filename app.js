...
let usersUnsub = null;
let myUserUnsub = null;
let presencesUnsub = null;
let mensalidadesUnsub = null;
    ...
    users.slice(0, 12).forEach((u) => {
      ...
      row.innerHTML = `
        ...
        <div class="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2">
          <button class="w-full rounded-xl px-3 py-2 text-sm font-semibold bg-zinc-700 hover:bg-zinc-600" data-edit-user="${escapeHtml(u.uid || "")}">
            Editar cadastro
          </button>
      ...
      `;
      row.querySelector('button[data-edit-user]')?.addEventListener("click", () => editUser(u.uid));
...
async function savePublicProfile() {
  ...
  const profile = {
    ...
    foto,
    observacao,
    presencas: Number(existing.presencas || 0),
    status: normalizeStatus(existing.status || (targetUid === state.currentUser.uid ? (state.profile?.status || "pendente") : "pendente")),
    role: existing.role || "aluno",
    plano: existing.plano || "",
    valorBase: existing.valorBase || 0,
    descontoPercent: existing.descontoPercent || 0,
    descontoValor: existing.descontoValor || 0,
    valorFinal: existing.valorFinal || 0,
    grupoFamiliar: existing.grupoFamiliar || "",
    totalPass: existing.totalPass || false,
    vencimento: existing.vencimento || "",
    createdAt: existing.createdAt || serverTimestamp(),
    updatedAt: serverTimestamp()
  };

  await setDoc(targetRef, profile, { merge: true });
  ...
  if (targetUid === state.currentUser.uid) {
    ...
    alert("Cadastro do aluno atualizado.");
    show("admin");
  }
}
...
function refreshProfileEditorUi() {
  const saveBtn = document.getElementById("savePublicProfileBtn");
  if (saveBtn) {
    saveBtn.textContent = canAccessAdmin() && state.editingUid ? "Salvar alterações" : "Salvar cadastro";
  }
}
...
function bindButtons() {
  ...
  document.getElementById("backFromAdmin")?.addEventListener("click", () => { clearEditMode(); show("home"); });
  document.getElementById("backFromCard")?.addEventListener("click", () => { clearEditMode(); show("home"); });
  document.getElementById("goCompleteProfileBtn")?.addEventListener("click", () => { clearEditMode(); fillProfileForm(state.profile || {}); show("create"); });
  ...
  document.getElementById("savePublicProfileBtn")?.addEventListener("click", async () => { try { await savePublicProfile(); } catch (error) { alert("Erro ao salvar cadastro: " + (error?.message || error)); } finally { refreshProfileEditorUi(); } });
  ...
  el.pendingList?.addEventListener("click", async (event) => {
    const editBtn = event.target.closest("button[data-edit-user]");
    if (editBtn) {
      try { await editUser(editBtn.dataset.editUser); } catch (error) { alert("Erro ao abrir cadastro: " + (error?.message || error)); }
      return;
    }
...
(function init() {
  ...
  if (canAccessAdmin()) {
    ...
    getDoc(doc(db, "users", state.currentUser.uid)).then((snap) => {
      if (snap.exists()) {
        state.profile = { uid: snap.id, ...snap.data() };
        renderProfilePreview(state.profile);
        renderStudentArea(state.profile);
        updateCard(state.profile);
        renderHomeSummary();
