...
const el = {
  ...
  avisosCount: document.getElementById("avisosCount")
};
...
function priorityClasses(priority) {
  if (priority === "urgente") return "bg-red-500/10 border-red-500/20 text-red-300";
  if (priority === "importante") return "bg-yellow-500/10 border-yellow-500/20 text-yellow-300";
...
function renderAvisos() {
  ...
  if (el.homeAvisosCount) {
    el.homeAvisosCount.textContent = `${avisos.length} aviso${avisos.length === 1 ? "" : "s"}`;
  }
  if (el.avisosCount) {
    el.avisosCount.textContent = `${avisos.length} aviso${avisos.length === 1 ? "" : "s"}`;
  }

  if (el.homeAvisosList) {
    if (!latest.length) {
      el.homeAvisosList.innerHTML = '<div class="text-zinc-500 text-sm">Ainda não há avisos publicados.</div>';
    } else {
      el.homeAvisosList.innerHTML = latest.map((a) => `
        <div class="panel-soft rounded-2xl p-4 border ${priorityClasses(a.prioridade || "normal")}">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <div class="text-xs font-semibold uppercase tracking-[0.2em] ${a.prioridade === "urgente" ? "text-red-300" : a.prioridade === "importante" ? "text-yellow-300" : "text-zinc-400"}">
                ${escapeHtml(priorityLabel(a.prioridade || "normal"))}
              </div>
              <div class="mt-1 font-semibold text-white">${escapeHtml(a.titulo || "-")}</div>
            </div>
            <div class="text-red-400 text-lg">📢</div>
          </div>
          <div class="mt-3 text-zinc-300 text-sm leading-relaxed whitespace-pre-line">${escapeHtml(a.mensagem || "")}</div>
        </div>
      `).join("");
    }
  }
  ...
  if (el.avisosList) {
    if (!avisos.length) {
      el.avisosList.innerHTML = '<div class="text-zinc-500 text-sm">Nenhum aviso publicado ainda.</div>';
    } else {
      el.avisosList.innerHTML = avisos.map((a) => `
        <div class="bg-zinc-800/80 border border-zinc-700 rounded-2xl p-3 shadow-sm">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <div class="text-xs font-semibold uppercase tracking-[0.2em] ${a.prioridade === "urgente" ? "text-red-300" : a.prioridade === "importante" ? "text-yellow-300" : "text-zinc-400"}">
                ${escapeHtml(priorityLabel(a.prioridade || "normal"))}
              ...
              <div class="mt-1 font-semibold text-white">${escapeHtml(a.titulo || "-")}</div>
              <div class="text-zinc-400 text-xs mt-1">${a.createdAt?.toDate ? a.createdAt.toDate().toLocaleString("pt-BR") : ""}</div>
            </div>
            <button class="bg-red-700 hover:bg-red-600 rounded-xl px-3 py-2 text-sm font-semibold" data-delete-aviso="${escapeHtml(a.id || "")}">
              Excluir
            </button>
          </div>
          <div class="mt-3 text-zinc-300 text-sm leading-relaxed whitespace-pre-line">${escapeHtml(a.mensagem || "")}</div>
...
async function saveAviso() {
  ...
  const prioridade = el.avisoPrioridade?.value || "normal";
  if (!titulo || !mensagem) return alert("Preencha título e mensagem do aviso.");
...
function renderFinance() {
  ...
  state.mensalidades.forEach((f) => {
    ...
    if (status === "atrasado") totalAtrasado += valor;
    ...
    card.innerHTML = `
      <div class="flex items-start justify-between gap-3">
        <div>
          ...
          <div class="text-zinc-300">Status: <span class="${(f.status || "pendente") === "pago" ? "text-emerald-400" : (f.status || "pendente") === "atrasado" ? "text-red-400" : "text-yellow-400"}">${escapeHtml(f.status || "pendente")}</span></div>
        </div>
...
function bindButtons() {
  ...
  document.getElementById("saveAvisoBtn")?.addEventListener("click", async () => {
    try { await saveAviso(); } catch (error) { alert("Erro ao publicar aviso: " + (error?.message || error)); }
  });
...
function watchCollections() {
  ...
  onSnapshot(query(collection(db, "avisos"), orderBy("createdAt", "desc")), (snapshot) => {
