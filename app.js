...
const classSchedules = [
  { id: "seg-1500", day: "Segunda-feira", className: "Kung Fu", time: "15:00 às 16:00" },
  { id: "seg-1630", day: "Segunda-feira", className: "Kung Fu", time: "16:30 às 17:30" },
  { id: "seg-1800", day: "Segunda-feira", className: "Kung Fu", time: "18:00 às 19:00" },
  { id: "ter-1000", day: "Terça-feira", className: "Kung Fu", time: "10:00 às 11:00" },
  { id: "ter-2030", day: "Terça-feira", className: "Kung Fu", time: "20:30 às 21:30" },
  { id: "qua-1500", day: "Quarta-feira", className: "Kung Fu", time: "15:00 às 16:00" },
  { id: "qua-1630", day: "Quarta-feira", className: "Kung Fu", time: "16:30 às 17:30" },
  { id: "qua-1800", day: "Quarta-feira", className: "Tai Chi", time: "18:00 às 19:00" },
  { id: "qua-1930", day: "Quarta-feira", className: "Kung Fu", time: "19:30 às 20:30" },
  { id: "qua-2100", day: "Quarta-feira", className: "Boxe Chinês", time: "21:00 às 22:00" },
  { id: "qui-1000", day: "Quinta-feira", className: "Kung Fu", time: "10:00 às 11:00" },
  { id: "qui-2030", day: "Quinta-feira", className: "Kung Fu", time: "20:30 às 21:30" },
  { id: "sex-1500", day: "Sexta-feira", className: "Kung Fu", time: "15:00 às 16:00" },
  { id: "sex-1630", day: "Sexta-feira", className: "Kung Fu", time: "16:30 às 17:30" },
  { id: "sex-1800", day: "Sexta-feira", className: "Tai Chi", time: "18:00 às 19:00" },
  { id: "sex-1930", day: "Sexta-feira", className: "Kung Fu", time: "19:30 às 20:30" },
  { id: "sex-2100", day: "Sexta-feira", className: "Boxe Chinês", time: "21:00 às 22:00" },
  { id: "sab-1115", day: "Sábado", className: "Kung Fu", time: "11:15 às 12:15" }
];

const scheduleDaysOrder = ["Segunda-feira", "Terça-feira", "Quarta-feira", "Quinta-feira", "Sexta-feira", "Sábado"];
...
const el = {
  ...
  presenceSchedule: document.getElementById("presenceSchedule"),
...
function setBankStatus(text) { if (el.statBanco) el.statBanco.textContent = text; }
...
function renderSchedules() {
  ...
  const grouped = classSchedules.reduce((acc, item) => {
  ...
  scheduleDaysOrder.forEach((day) => {
    const items = grouped[day] || [];
    const card = document.createElement("div");
    card.className = "bg-zinc-950 border border-zinc-800 rounded-2xl p-4";
    card.innerHTML = `
      <div class="text-orange-400 font-black text-lg mb-3">${escapeHtml(day)}</div>
      <div class="space-y-2">
        ${items.map((i) => `<div class="bg-zinc-800 rounded-xl px-3 py-2 border border-zinc-700"><b>${escapeHtml(i.className)}</b> — ${escapeHtml(i.time)}</div>`).join("") || '<div class="text-zinc-400">Sem aulas.</div>'}
      </div>
    `;
    el.scheduleGrid.appendChild(card);
  });
}
...
function cardQrText(profile) { return profile ? `${profile.uid}|${profile.nome || ""}|${profile.faixa || ""}|${profile.status || ""}` : "sem-dados"; }
function updateCard(profile) {
  ...
  if (window.QRCode && el.qrcode) {
    el.qrcode.innerHTML = "";
    new QRCode(el.qrcode, { text: cardQrText(profile), width: 160, height: 160, colorDark: "#111111", colorLight: "#ffffff", correctLevel: QRCode.CorrectLevel.M });
  }
...
function renderUsersLists() {
  ...
  state.pending.forEach((u) => {
    ...
    card.innerHTML = `<div class="font-bold text-yellow-400">${escapeHtml(u.nome || "Sem nome")}</div><div class="text-zinc-300 text-sm">E-mail: ${escapeHtml(u.email || "-")}</div><div class="text-zinc-300 text-sm">Telefone: ${escapeHtml(u.telefone || "-")}</div><div class="text-zinc-300 text-sm">Faixa: ${escapeHtml(u.faixa || "-")}</div><button data-approve="${u.uid}" class="mt-3 w-full bg-emerald-700 hover:bg-emerald-600 px-4 py-3 rounded-xl font-semibold">Aprovar aluno</button>`;
    el.pendingList.appendChild(card);
  });
  state.approved.forEach((u) => {
    const card = document.createElement("div");
    card.className = "bg-zinc-800 rounded-2xl p-4 border border-zinc-700";
    card.innerHTML = `<div class="font-bold text-emerald-400">${escapeHtml(u.nome || "Sem nome")}</div><div class="text-zinc-300 text-sm">E-mail: ${escapeHtml(u.email || "-")}</div><div class="text-zinc-300 text-sm">Telefone: ${escapeHtml(u.telefone || "-")}</div><div class="text-zinc-300 text-sm">Faixa: ${escapeHtml(u.faixa || "-")}</div><div class="text-zinc-300 text-sm">Presenças: ${Number(u.presencas || 0)}</div><button data-revoke="${u.uid}" class="mt-3 w-full bg-zinc-700 hover:bg-zinc-600 px-4 py-3 rounded-xl font-semibold">Voltar para pendente</button>`;
    el.approvedList.appendChild(card);
  });
  el.presenceAluno.innerHTML = "";
  el.financeAluno.innerHTML = "";
  state.approved.forEach((u) => {
    const o1 = document.createElement("option"); o1.value = u.uid; o1.textContent = u.nome || "Sem nome"; el.presenceAluno.appendChild(o1);
    const o2 = document.createElement("option"); o2.value = u.uid; o2.textContent = u.nome || "Sem nome"; el.financeAluno.appendChild(o2);
  });
  renderPresences();
  renderFinance();
}
function renderPresences() {
  ...
  state.presences.slice(0, 10).forEach((p) => {
    ...
    const schedule = classSchedules.find(s => s.id === p.scheduleId);
    card.innerHTML = `<div class="font-semibold text-emerald-400">${escapeHtml(aluno?.nome || "Aluno removido")}</div><div class="text-zinc-300">Data: ${escapeHtml(p.data || "-")}</div><div class="text-zinc-300">Horário: ${escapeHtml(p.hora || "-")}</div><div class="text-zinc-300">Aula: ${escapeHtml(schedule ? `${schedule.day} • ${schedule.className} • ${schedule.time}` : p.scheduleLabel || "-")}</div>`;
...
function renderFinance() {
  ...
  state.mensalidades.forEach((f) => {
    ...
    const card = document.createElement("div");
    card.className = "bg-zinc-800 rounded-2xl p-3 border border-zinc-700 text-sm";
    card.innerHTML = `<div class="flex items-start justify-between gap-3"><div><div class="font-semibold text-purple-400">${escapeHtml(aluno?.nome || "Aluno removido")}</div><div class="text-zinc-300">Valor: ${money(f.valor)}</div><div class="text-zinc-300">Vencimento: ${escapeHtml(f.vencimento || "-")}</div><div class="text-zinc-300">Status: <span class="${(f.status || "pendente") === "pago" ? "text-emerald-400" : "text-yellow-400"}">${escapeHtml(f.status || "pendente")}</span></div></div><button data-pay="${f.id}" class="bg-emerald-700 hover:bg-emerald-600 px-3 py-2 rounded-xl font-semibold text-sm">Marcar pago</button></div>`;
...
async function loadUserProfile(user) {
  ...
  if (snap.exists()) { state.profile = { uid: user.uid, ...snap.data() }; state.role = state.profile.role || "aluno"; } else { state.profile = null; state.role = "aluno"; }
...
async function createAccount() {
  ...
  try {
    ...
    await setDoc(doc(db, "users", cred.user.uid), { uid: cred.user.uid, email, nome: "", telefone: "", faixa: "", foto: "", observacao: "", status: "pendente", role: "aluno", presencas: 0, createdAt: serverTimestamp() }, { merge: true });
    alert("Conta criada. Agora complete o cadastro.");
    show("create");
  } catch (error) { alert("Erro ao criar conta: " + (error?.message || error)); }
}
async function loginEmail() {
  const email = document.getElementById("emailLogin").value.trim();
  const password = document.getElementById("passLogin").value.trim();
  if (!email || !password) return alert("Preencha e-mail e senha.");
  try { await signInWithEmailAndPassword(auth, email, password); alert("Login realizado."); } catch (error) { alert("Erro ao entrar: " + (error?.message || error)); }
}
...
async function savePublicProfile() {
  ...
  try { foto = await uploadPhotoIfAny(state.currentUser.uid); } catch (error) { alert("Falha ao enviar foto: " + (error?.message || error)); return; }
  const profile = { uid: state.currentUser.uid, email: state.currentUser.email || "", nome, telefone, faixa, foto, observacao, status: "pendente", role: "aluno", presencas: state.profile?.presencas || 0, updatedAt: serverTimestamp() };
  await setDoc(doc(db, "users", state.currentUser.uid), profile, { merge: true });
  state.profile = { ...state.profile, ...profile };
  renderProfilePreview(state.profile); renderStudentArea(state.profile); updateCard(state.profile);
  alert("Cadastro salvo. Aguarde aprovação do admin.");
  show("aluno");
}
async function approveUser(uid) { await updateDoc(doc(db, "users", uid), { status: "aprovado", role: "aluno" }); }
async function revokeUser(uid) { await updateDoc(doc(db, "users", uid), { status: "pendente", role: "aluno" }); }
async function markPresence() {
  ...
  await addDoc(collection(db, "presencas"), { alunoId, data, hora, createdAt: serverTimestamp() });
...
async function saveFinance() {
  ...
  el.financeValor.value = ""; el.financeVencimento.value = ""; el.financeStatus.value = "pendente";
...
function bindButtons() {
  ...
  document.getElementById("btnCarteirinha").addEventListener("click", () => { if (!state.currentUser) return alert("Faça login primeiro."); show("card"); });
  ...
  document.getElementById("btnLogout").addEventListener("click", async () => { localStorage.removeItem("adminUnlocked"); state.adminUnlocked = false; await signOut(auth); state.role = "guest"; });
  ...
  el.pendingList.addEventListener("click", async (event) => { const btn = event.target.closest("button[data-approve]"); if (!btn) return; try { await approveUser(btn.dataset.approve); } catch (error) { alert("Erro ao aprovar: " + (error?.message || error)); } });
  el.approvedList.addEventListener("click", async (event) => { const btn = event.target.closest("button[data-revoke]"); if (!btn) return; try { await revokeUser(btn.dataset.revoke); } catch (error) { alert("Erro ao voltar para pendente: " + (error?.message || error)); } });
  el.financeList.addEventListener("click", async (event) => { const btn = event.target.closest("button[data-pay]"); if (!btn) return; try { await updateDoc(doc(db, "mensalidades", btn.dataset.pay), { status: "pago" }); } catch (error) { alert("Erro ao marcar pago: " + (error?.message || error)); } });
}
function watchCollections() {
  ...
  onSnapshot(query(collection(db, "presencas"), orderBy("createdAt", "desc")), (snapshot) => { state.presences = snapshot.docs.map(d => ({ id: d.id, ...d.data() })); renderPresences(); }, () => {});
  onSnapshot(query(collection(db, "mensalidades"), orderBy("createdAt", "desc")), (snapshot) => { state.mensalidades = snapshot.docs.map(d => ({ id: d.id, ...d.data() })); renderFinance(); }, () => {});
}
onAuthStateChanged(auth, async (user) => {
  ...
  if (localStorage.getItem("adminUnlocked") === "1") { state.role = "admin"; show("admin"); } else { show("aluno"); }
});
(function init() {
  bindButtons();
  renderSchedules();
  ...
  if (el.btnInstall && "beforeinstallprompt" in window) {
    ...
    window.addEventListener("beforeinstallprompt", (event) => { event.preventDefault(); deferredPrompt = event; el.btnInstall.classList.remove("hidden"); });
