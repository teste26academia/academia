...
async function ensureRecaptcha() {
  ...
  try {
    ...
    reportFirebaseError("Falha ao criar reCAPTCHA", error);
    throw error;
  }
}
...
async function loadData() {
  try {
    ...
    state.alunos = alunosSnap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
    ...
    onSnapshot(collection(db, "alunos"), snap => {
      state.alunos = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
      render();
    }, err => {
      console.error(err);
...
async function saveFinance() {
  ...
  try {
    await addDoc(collection(db, "financeiro"), {
      alunoId, valor, vencimento, status,
      createdAt: serverTimestamp()
    });
    el.finValor.value = "";
...
window.addEventListener("error", ev => {
  debug(`Erro global: ${ev.error?.message || ev.message}`, "error");
});
window.addEventListener("unhandledrejection", ev => {
  debug(`Promise rejeitada: ${ev.reason?.message || ev.reason}`, "error");
});
